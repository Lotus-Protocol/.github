![Lotus Header](lotus_header.jpeg)

## Lotus

Lotus is an onchain lending protocol that lets lenders and borrowers meet on a risk curve inside a single market. Instead of creating separate pools for every risk setting, Lotus uses tranches to offer multiple risk levels while keeping liquidity connected.

## Resources

- 🌐 **App**: [lotuslabs.net](https://www.lotuslabs.net/)
- 📖 **Documentation**: [docs.lotuslabs.net](https://docs.lotuslabs.net/)
- ✍️ **Blog**: [lotuslabs.net/blog](https://www.lotuslabs.net/blog)

## Community

- 💬 [Get in Touch](https://www.lotuslabs.net/#form)
- 🐦 [X (Twitter)](https://x.com/LotusFi_)
- 💼 [LinkedIn](https://www.linkedin.com/company/lotusfi/)
